...
namespace ChatBot
{
    ...
    public class CBR
    {
        ...
        {
            string[] parsed_a = Parser.ParseEnglish(A);
         ...
         /// <param name="A">사용자가 말한 문장.</param>
         /// <param name="B">응답한 문장.</param>
        public void AddEnglishConversation(string A, string B)
        {
            string[] parsed_a = Parser.ParseEnglish(A);
        ...
        /// <summary>
        /// 입력을 받고, 입력에 대한 출력과 새로운 학습을 합니다.
        /// </summary>
        /// <param name="input">입력</param>
        /// <param name="tokens">나눠진 토큰</param>
        /// <returns>출력</returns>
        public string Eval(string input, string[] tokens)
        {
            //TODO: 메서드 분할
            ...
            foreach(string s in tokens)
            {
                ...
                bool isIn = false;
                foreach(string key in Table.table.Keys)
                {
                    if (s == key) isIn = true;
                }

                if (isIn == false)
                    AddConversation(s, result);
            }
